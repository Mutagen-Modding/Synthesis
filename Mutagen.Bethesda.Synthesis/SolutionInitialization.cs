...
namespace Mutagen.Bethesda.Synthesis
{
    ...
    {
        public static GetResponse<string> ValidateProjectPath(
            ...
            GetResponse<string> sln)
        {
            ...
            try
            {
                ...
                if (fileSystem.File.Exists(projPath))
                {
                    return GetResponse<string>.Fail($"Target project folder cannot already exist as a file: {projPath}");
                }
                if (fileSystem.Directory.Exists(projPath)
                    && (fileSystem.Directory.EnumerateFiles(projPath).Any()
                        || fileSystem.Directory.EnumerateDirectories(projPath).Any()))
                {
        ...
        public static string[] CreateSolutionFile(
            IFileSystem fileSystem,
            string solutionPath)
        {
            var slnDir = Path.GetDirectoryName(solutionPath)!;
            fileSystem.Directory.CreateDirectory(slnDir);
            ...
            fg.AppendLine("</Project>");
            fg.Generate(Path.Combine(slnDir, "Directory.Build.props"), fileSystem: fileSystem);
        ...
        {
            var projName = Path.GetFileNameWithoutExtension(projPath);
            fileSystem.File.AppendAllLines(solutionpath,
        ...
        public static void GenerateGitIgnore(
            ...
            string folder)
        {
            fileSystem.File.WriteAllText(Path.Combine(folder, ".gitignore"),
